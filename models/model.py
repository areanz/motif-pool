    ...
    def forward(self, batch_graph):
        ...
        for pool_layer in range(self.num_pool_layers):
            ...
            for i, graph in enumerate(batch_graph):
                ...
                start_idx.append(start_idx[i] + len(graph.g_list[pool_layer*2]))
            # print("start", start_idx)
            edge_index, x = self.__preprocess(batch_graph, pool_layer*2, start_idx)
            for i, graph in enumerate(batch_graph):
                graph.mid_feature = x[start_idx[i]:start_idx[i + 1]]

            start_idx = [0]
            for i, graph in enumerate(batch_graph):
                start_idx.append(start_idx[i] + len(graph.g_list[pool_layer*2+1]))
            # print("start", start_idx)
            edge_index, x = self.__preprocess(batch_graph, pool_layer*2+1, start_idx)
